...
export default function PostsHistoryPage() {
  ...
  const fetchPosts = async (page = 1) => {
    try {
      ...
      if (data?.success) {
        ...
        const errorMsg = data?.message || 'Failed to fetch posts';
        console.error('Failed to fetch posts:', errorMsg);
        setError(errorMsg);
        setPosts([]);
      }
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-[96%] 2xl:max-w-[95%] mx-auto px-2 sm:px-4 py-4 sm:py-6 space-y-6">
      {/* Header */}
        ...
        <CardContent>
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="text-sm text-muted-foreground">
              {posts ? `Showing ${posts.length} of ${pagination.totalCount} posts` : 'Loading posts...'}
            </div>
